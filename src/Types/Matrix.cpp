...
template <int Rows, int Cols> Matrix<Rows, Cols>::Matrix(const Matrix& m)
{
    for (int i = 0; i < Rows; ++i) {
        for (int j = 0; j < Cols; ++j) {
...
Matrix<Rows, Cols2>
Matrix<Rows, Cols>::operator*(const Matrix<Rows2, Cols2>& m) const
    requires(Cols == Rows2)
{
    ...
    requires(Rows == 4 && Cols == 1)
{
    ...
    for (int i = 0; i < Rows; ++i) {
        result += values.at(i) * vector.values.at(i);
    }
    ...
    requires(Rows == 4 && Cols == 1)
{
    ...
    temp.getValue(0, 0)
        = (cGetY() * vector.cGetZ()) - (cGetZ() * vector.cGetY());
    temp.getValue(1, 0)
        = (cGetZ() * vector.cGetX()) - (cGetX() * vector.cGetZ());
    temp.getValue(2, 0)
        = (cGetX() * vector.cGetY()) - (cGetY() * vector.cGetX());
    //    temp.getValue(3, 0) = 1;
    temp.getValue(3, 0) = 0;
    ...
    requires(Rows == 4 && Cols == 1)
{
    ...
    length = sqrt(pow(values[0], 2) + pow(values[1], 2) + pow(values[2], 2));
...
template <int Rows, int Cols>
Matrix<4, 4>
Matrix<Rows, Cols>::getRotateConvert(const AxisName axis, const double angle)
{
...
template <int Rows, int Cols>
Matrix<4, 4> Matrix<Rows, Cols>::getViewConvert(
    const Vector<4>& eye, const Vector<4>& target, const Vector<4>& up)
{
...
template <int Rows, int Cols>
Matrix<4, 4> Matrix<Rows, Cols>::getProjectionConvert(
    const double fov,
    const double aspect,
    const double zFar,
    const double zNear)
...
template <int Rows, int Cols>
Matrix<4, 4> Matrix<Rows, Cols>::getViewportConvert(
    const double width,
    const double height,
    const double xMin,
    const double yMin)
...
template <int Rows, int Cols> void Matrix<Rows, Cols>::log() const
{
...
template <> void Vector<4>::log() const
{
    for (auto i = 0; i < 4; ++i) {
        std::cout << std::format("{:5.3f}", values.at(i)) << ' ';
    }
