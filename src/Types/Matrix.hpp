...
public:
    ...
    Matrix(double x, double y, double z, double w = 1.F)
    ...
    static Matrix<4, 4> getMoveConvert(const Matrix<4, 1>& translation);

    static Matrix<4, 4> getScaleConvert(const Matrix<4, 1>& scale);

    static Matrix<4, 4> getRotateConvert(AxisName axis, double angle);
    ...
    static Matrix<4, 4>
    getProjectionConvert(double fov, double aspect, double zFar, double zNear);

    static Matrix<4, 4>
    getViewportConvert(double width, double height, double xMin, double yMin);
...
template <int Rows, int Cols>
inline const double&
Matrix<Rows, Cols>::cGetValue(const int i, const int j) const
{
    return values[i * Cols + j];
}
...
template <int Rows> using Vector = Matrix<Rows, 1>;
